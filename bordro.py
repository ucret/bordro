...
emek = st.number_input(label= "Aylık Ücret", step=1, value=0)
...
for i in range(12):
    ...
    munzam[i] = round((emek + mt.ceil(emek/3))*0.07,2)
    net[i] = round(Aylık[i]-(sske[i]+sski[i]+dv[i]+gv[i]) + igv[i] + idv[i]-munzam[i])
    

dic = {"Aylık": Aylık,"Emekli Sandık":sske,"İşsizlik Payı":sski,"Gelir Vergi":gv,"Damga İstisna":idv,"vergi istisnası": igv,"munzam pay": munzam,"Net Tutar": net}
